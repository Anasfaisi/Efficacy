...
export class PlanController{
    ...
    let result = await this._planService.createPlan(req.body);
    ...
    async getAllPlans(req: Request, res: Response) {
        ...
        let result = await this._planService.getAllPlans(page, limit, search as string, status as string);
        res.status(httpStatus.OK).json(result)
    }
    async getPlanById(req:Request,res:Response){
        let result = await this._planService.getPlanById(req.params.planId);
        res.status(httpStatus.OK).json(result)
    }
    async updatePlan(req:Request,res:Response){
        await this._planService.updatePlan(req.params.planId,req.body);
        res.status(httpStatus.OK).json({ message: "Plan updated successfully" })
    }
