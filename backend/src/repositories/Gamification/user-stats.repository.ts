...
export class UserStatsRepository extends BaseRepository<IUserStats> implements IUserStatsRepository{
    constructor() {
        super(UserStats)
  }
    async CreateUserStats(data: Partial<UserStatsEntity>): Promise<UserStatsEntity> {
    ...
    async UpdateUserStats(userStatsId: string, data: UserStatsEntity): Promise<UserStatsEntity| null> {
        const UserStatsData = UserStatsMapper.toPersistence(data);
