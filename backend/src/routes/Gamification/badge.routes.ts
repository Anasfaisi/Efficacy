...
export default function BadgeRoutes(badgeController : BadgeController , tokenService : ITokenService){
    ...
    router.get('/',authenticateAndAuthorize(tokenService,[Role.Admin,Role.User]),asyncWrapper(badgeController.getAllBadges.bind(badgeController)))
    router.get('/user',authenticateAndAuthorize(tokenService,[Role.User]),asyncWrapper(badgeController.getUserBadges.bind(badgeController)))
