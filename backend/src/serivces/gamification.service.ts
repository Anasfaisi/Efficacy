...
export class GamificationService implements IGamificationService {
    constructor(
        ...
        private _userStatsRepository: IUserStatsRepository,
                ...
                async (payload) => {
                    try {
                        ...
                        if (event === GamificationEvent.TASK_COMPLETED) {
                            let stats =
                                await this._userStatsRepository.FindByUserId(
                                    userIdStr
                                );
                            if (!stats) {
                                stats =
                                    await this._userStatsRepository.CreateUserStats(
                                        {
                            ...
                            stats =
                                await this._dailyStreakCalculator.calculateDailyStreak(
                                    stats
                                );
                            await this._userStatsRepository.UpdateUserStats(
                                stats.id,
                                stats
                            );
                            ...
                            if (stats.taskStreakDays !== oldStreak) {
                                emitGamificationEvent(
                                    GamificationEvent.STREAK_UPDATED,
