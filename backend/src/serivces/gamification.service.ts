...
import { GamificationEvent, BadgeTemplate, IBadge } from '@/types/gamification.types';
...
export class GamificationService implements IGamificationService {
  ...
  private initializeListeners() {
    ...
    for (const event of events) {
      subscribeToGamificationEvent(event as GamificationEvent, async (payload) => {
        try {
          await this.evaluateBadges(new Types.ObjectId(payload.userId), event as GamificationEvent);
        } catch (error) {
          logger.error(`Error during Gamification evaluation for ${payload.userId} on ${event}:`, error);
        }
      });
    }
    logger.info("🎮 Gamification Service initialized and listening to events.");
  }

  public async evaluateBadges(userId: Types.ObjectId, event: GamificationEvent) {
    try {
      ...
      const unlockedBadgeIds = unlockedBadges.map(ub => (ub.badgeId as Types.ObjectId).toString());

      const badgesToEvaluate = eligibleBadges.filter(
        badge => !unlockedBadgeIds.includes((badge._id as Types.ObjectId).toString())
      );
      ...
      for (const badge of badgesToEvaluate) {
       ...
       logger.error(`Error evaluating badges for user ${userId} and event ${event}:`, error);
    }
  }
  ...
  private async unlockBadge(userId: Types.ObjectId, badgeId: Types.ObjectId) {
    try {
      ...
      if (error.code === 11000) {
        logger.warn(`Gamification: Badge ${badgeId} skipped for ${userId} (Already unlocked natively).`);
      } else {
         logger.error(`Error unlocking badge ${badgeId} for user ${userId}:`, error);
      }
    }
  }
  ...
  public async getBadgeById(id: string | Types.ObjectId): Promise<IBadge | null> {
    return await this._badgeRepository.findById(id.toString());
  }

  public async updateBadge(id: string | Types.ObjectId, updateData: Partial<IBadge>): Promise<IBadge | null> {
