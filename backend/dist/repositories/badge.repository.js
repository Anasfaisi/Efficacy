"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BadgeRepository = void 0;
const inversify_1 = require("inversify");
const base_repository_1 = require("./base.repository");
const badge_model_1 = require("@/models/badge.model");
const badge_mapper_1 = require("@/Mapper/badge.mapper");
let BadgeRepository = class BadgeRepository extends base_repository_1.BaseRepository {
    constructor() {
        super(badge_model_1.Badge);
    }
    async createBadge(badge) {
        const createdBadge = await this.model.create(badge);
        return badge_mapper_1.BadgeMapper.ToEntity(createdBadge);
    }
    async getAllBadgesAdmin(page, limit) {
        const skip = (page - 1) * limit;
        const total = await this.model.countDocuments();
        const badges = await this.model
            .find()
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit)
            .exec();
        return { badges, total };
    }
    async findBadges(query) {
        const badges = await super.find(query);
        return badge_mapper_1.BadgeMapper.listtoBadgeEntity(badges);
    }
};
exports.BadgeRepository = BadgeRepository;
exports.BadgeRepository = BadgeRepository = __decorate([
    (0, inversify_1.injectable)(),
    __metadata("design:paramtypes", [])
], BadgeRepository);
//# sourceMappingURL=badge.repository.js.map