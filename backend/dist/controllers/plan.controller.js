...
let PlanController = class PlanController {
    ...
    async createPlan(req, res) {
        ...
        res.status(200 /* httpStatus.OK */).json(result);
    }
    async getAllPlans(req, res) {
        const { search = '', status = 'all' } = req.query;
        const page = parseInt(req.query.page) || 1;
        const limit = parseInt(req.query.limit) || 10;
        const result = await this._planService.getAllPlans(page, limit, search, status);
        res.status(200 /* httpStatus.OK */).json(result);
    }
...
exports.PlanController = PlanController = __decorate([
    ...
    __param(0, (0, inversify_1.inject)(inversify_key_types_1.TYPES.PlanService)),
