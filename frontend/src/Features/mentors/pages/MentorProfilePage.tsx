...
const MentorProfilePage = () => {
  ...
  const handleSavePartial = async (fields: Partial<Mentor>, sectionId: string) => {
    try {
      // Validate with Zod
  ...
  const handlePasswordSave = async () => {
    try {
      ...
      await updateMentorProfileBasicInfo({ currentPassword, newPassword } as any);
  ...
  const handleMediaChange = async (file: File, type: 'profilePic' | 'coverPic') => {
    setIsLoading(type);
    try {
      ...
      toast.success(`${type === 'profilePic' ? 'Profile' : 'Cover'} picture updated`);
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-20">
      ...
      <div className="relative h-64 w-full bg-slate-200 overflow-hidden group">
        {fullMentor?.coverPic ? (
          <img src={fullMentor.coverPic} className="w-full h-full object-cover" alt="Cover" />
        ) : (
        ...
        <input 
            ...
            ref={coverPicRef} 
            className="hidden" 
            onChange={(e) => e.target.files?.[0] && handleMediaChange(e.target.files[0], 'coverPic')} 
        />
      </div>
          ...
          <div className="relative">
            <div className="w-36 h-36 rounded-full border-4 border-white overflow-hidden bg-slate-100 shadow-lg">
              {fullMentor?.profilePic ? (
                <img src={fullMentor.profilePic} alt={name} className="w-full h-full object-cover" />
              ) : (
            ...
            <input
              ...
              className="hidden"
              accept="image/*"
              onChange={(e) => e.target.files?.[0] && handleMediaChange(e.target.files[0], 'profilePic')}
            />
          </div>
        ...
        <div className="space-y-4">
          ...
          >
            <div className="space-y-6">
                ...
                <div>
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 mb-2 block">Professional Bio</label>
                    <textarea
                        value={bio}
          ...
          <ConfigSection
            ...
            onSave={() => handleSavePartial({ phone, city, country }, 'contact')}
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Email (Immutable)</label>
                    <input
                        type="email"
                        value={email}
                        disabled
          ...
          {fullMentor?.mentorType === 'Academic' && (
            <ConfigSection
                ...
                onSave={() => handleSavePartial({ qualification, university, graduationYear, academicSpan }, 'academic')}
            ...
            >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Graduation Year</label>
                        <input
                            type="text"
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Academic Span (Years)</label>
                        <input
                            type="text"
                            value={academicSpan}
                            onChange={(e) => setAcademicSpan(e.target.value)}
                            className="w-full bg-white border border-slate-200 rounded-lg px-4 py-2.5 text-slate-900 focus:ring-2 focus:ring-blue-100 focus:border-blue-500 outline-none transition-all"
                            placeholder="5"
                        />
                    </div>
          ...
          <ConfigSection
            ...
            onSave={() => handleSavePartial({ currentRole, industryCategory, skills, expertise }, 'industry')}
          ...
          >
            <div className="space-y-6">
                ...
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 mb-1 block">Industrial Skills (Knowledge)</label>
                    <input
                        type="text"
                        value={skills}
                ...
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 mb-1 block">Guidance Topics (Expertise)</label>
                    <textarea
                        value={expertise}
                    ...
                    />
                    <p className="text-[10px] text-slate-400 ml-1 mt-1 italic">Mention specific areas where you can provide strong mentorship.</p>
                </div>
            </div>
          </ConfigSection>
          ...
          <ConfigSection
            ...
            onSave={() => handleSavePartial({ monthlyCharge: Number(monthlyCharge) }, 'money')}
          ...
          <ConfigSection
            ...
            onSave={() => handleSavePartial({ linkedin, github, personalWebsite: website }, 'socials')}
