  ...
  />
      ...
      <div className="overflow-x-auto bg-white shadow-md rounded-lg">
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {mentors.map((m) => (
              <tr key={m.mentorId} className="hover:bg-gray-50">
                <td className="p-3 border">{m.mentorId}</td>
                <td className="p-3 border">{m.name}</td>
                <td className="p-3 border">{m.expertise}</td>
                <td className="p-3 border">{m.availability}</td>

                <td className="p-3 border">
                  <span
                    className={`px-3 py-1 text-xs rounded-full text-white ${
                      m.currentActive ? "bg-green-600" : "bg-red-500"
                    }`}
                  >
                    {m.currentActive ? "Active" : "Inactive"}
                  </span>
                </td>
