...
export default function PlanTable() {
    ...
    const handleToggleStatus = async (planId: string, currentStatus: boolean) => {
        try {
            ...
            toast.success(`Plan marked as ${!currentStatus ? 'Active' : 'Inactive'}`);
            fetchPlans(); 
        } catch (err) {
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden text-sm">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50 border-b border-gray-100">
                                <th className="px-6 py-4 font-semibold text-gray-600 uppercase tracking-wider text-xs">Plan Info</th>
                                <th className="px-6 py-4 font-semibold text-gray-600 uppercase tracking-wider text-xs">Pricing & Cycle</th>
                                <th className="px-6 py-4 font-semibold text-gray-600 uppercase tracking-wider text-xs hidden md:table-cell">Features</th>
                                ...
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center">
                                        <div className="max-w-xs mx-auto">
                                            ...
                                            <h3 className="text-gray-900 font-medium mb-1">No plans found</h3>
                                    ...
                                    <tr key={plan._id} className="hover:bg-gray-50/50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 align-top">
                                            <div className="flex flex-col gap-1">
                                                ...
                                                {plan.mentorType && plan.mentorType !== 'None' && (
                                        ...
                                        <td className="px-6 py-4 align-top">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-gray-500 text-xs">
                                                    per {plan.billingCycleDays} days
                                                </span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 align-top hidden md:table-cell">
                                            <div className="flex flex-col gap-1 max-w-xs">
                                                <div className="text-gray-700 mb-1">
                                                    <span className="font-medium">{plan.features.length}</span> included features
                                                </div>
                                                <div className="flex flex-wrap gap-1.5">
                                                    {plan.features.slice(0, 2).map((feat, i) => (
                                                        <span key={i} className="px-2 py-1 bg-gray-100 border border-gray-200 rounded-md text-[11px] text-gray-600 truncate max-w-[120px]">
                                                            {feat}
                                                        </span>
                                                    ))}
                                        ...
                                        <td className="px-6 py-4 align-top text-right">
                                            <div className="flex items-center justify-end gap-2 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => openEditModal(plan)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit Plan">
                                                    <Edit size={16} />
                                                </button>
                                                <button onClick={() => handleToggleStatus(plan._id, plan.isActive)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title={plan.isActive ? "Deactivate Plan" : "Activate Plan"}>
                                                    {plan.isActive ? <Trash2 size={16} /> : <CheckCircle2 size={16} />}
                                                </button>
                ...
                <div className="px-6 py-4 bg-gray-50/50 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="text-gray-500 text-sm">
                        Showing <span className="font-medium text-gray-900">{plans.length}</span> of <span className="font-medium text-gray-900">{totalItems}</span> plans
                    </div>
