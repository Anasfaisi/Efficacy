...
export default function MentorDetailsPage() {
  ...
  useEffect(() => {
    const fetchMentor = async () => {
      ...
      try {
        ...
        const errorMessage = (err as any).response?.data?.message || 'Failed to load mentor details.';
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      toast.success(`Mentor ${newStatus === 'active' ? 'unblocked' : 'blocked'} successfully`);
    } catch (error) {
    ...
    <div className="max-w-6xl mx-auto min-h-screen pb-12 p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4 mb-8">
        ...
        <div className="md:ml-auto flex items-center gap-3">
           ...
           >
             {mentor.status === 'active' ? <UserX size={16} /> : <UserCheck size={16} />}
             {mentor.status === 'active' ? 'Block Mentor' : 'Unblock Mentor'}
           </button>
        </div>
      </div>
          ...
          <section className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1">
                ...
                <p className="text-gray-900 font-medium">{mentor.name}</p>
              </div>
              <div className="space-y-1">
                <label className="text-xs font-semibold text-gray-400 uppercase tracking-wide">
                  Email
             ...
             */}
             <div className="space-y-6">
               <div>
                 ...
                 <div className="flex flex-wrap gap-2">
                   {Array.isArray(mentor.availableDays) ? mentor.availableDays.map((day: string) => (
                     <span key={day} className="px-2 py-1 bg-green-50 text-green-700 text-xs font-semibold rounded">
                       {day}
                     </span>
                   )) : (typeof mentor.availableDays === 'string' ? mentor.availableDays.split(',').map(d => (
                      <span key={d} className="px-2 py-1 bg-green-50 text-green-700 text-xs font-semibold rounded">{d}</span>
                   )) : 'N/A')}
                 </div>
               </div>
             </div>
            ...
            <div className="space-y-3">
              ...
              {mentor.github && (
               ...
               {mentor.demoVideoLink && (
                ...
                >
                  <Video size={18} />
                  <span className="text-sm font-medium">Meet Video</span>
                </a>
              )}
            </div>
          </section>
            ...
            <ul className="space-y-3">
              {mentor.resume && (
                <li className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer group">
                  ...
                  <a href={mentor.resume} target="_blank" rel="noreferrer" className="text-xs text-blue-600 font-bold hover:underline">
                    View
                  </a>
                </li>
              )}
