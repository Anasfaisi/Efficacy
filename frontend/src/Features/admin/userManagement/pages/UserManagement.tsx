...
const UserManagement = () => {
    ...
    const fetchUsers = useCallback(async () => {
        try {
            setLoading(true);
            const data = await adminService.getAllUsers(currentPage, limit, debouncedSearch);
            setUsers(data.users);
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Manage platform users and their access status</p>
                </div>
                ...
                {!loading && users.length > 0 && totalPages > 1 && (
                    <div className="px-6 py-4 bg-gray-50/50 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="text-sm text-gray-500">
                            Showing <span className="font-medium text-gray-900">{users.length}</span> of <span className="font-medium text-gray-900">{totalCount}</span> users
                        </div>
                        <div className="flex items-center gap-2">
                            <button
