...
export function OTPPage() {
    const { tempEmail, isLoading, currentUser, resendAvailableAt, role } = useAppSelector((state) => state.auth);
    ...
    const handleResend = async () => {
        try {
            ...
            const message = typeof error === 'string' ? error : 'Failed to resend OTP';
            toast.error(message);
        }
    };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-mesh animate-gradient-slow p-4">
            ...
            >
                ...
                <h2 className="text-3xl font-black text-slate-800 mb-3">Verify Your Email</h2>
                <div className="flex items-center justify-center gap-2 text-slate-500 mb-10">
                    <Mail size={16} />
                    <p className="text-sm">Sent to <span className="font-bold text-slate-700">{tempEmail}</span></p>
                </div>
                ...
                <div className="pt-6 border-t border-slate-100">
                    {timer > 0 ? (
                        <p className="text-slate-500 text-sm">
                            Resend available in <span className="font-bold text-primary">{timer}s</span>
                        </p>
                    ) : (
                        <button
