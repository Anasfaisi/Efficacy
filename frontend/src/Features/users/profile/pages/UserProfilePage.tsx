...
const UserProfilePage = () => {
  ...
  const handlePasswordSave = async () => {
    try {
      ...
      await updateProfile({ currentPassword, newPassword } as any, currentUser?.id);
  ...
  const handleMediaChange = async (file: File) => {
    ...
    try {
      const res = await updateProfilePicture(file, 'user', currentUser?.id);
      toast.success('Profile picture updated');
      
      if (res.user.profilePic) {
        dispatch(updateCurrentUser({ profilePic: res.user.profilePic }));
      }
    } catch (error) {
    ...
    <div className="flex min-h-screen bg-[#FDFCFE]">
      ...
      <div className="flex-1 min-h-screen overflow-y-auto">
        ...
        <div className="bg-white border-b border-purple-100 px-6 py-3 flex items-center gap-2 text-sm">
          ...
          <span className="text-slate-500 font-medium cursor-pointer hover:text-purple-600 transition-colors">Profile</span>
          <ChevronRight className="w-3 h-3 text-slate-300" />
          <span className="text-purple-600 font-bold capitalize">{activeTab}</span>
        </div>
            ...
            <div className="flex flex-col md:flex-row items-center gap-8">
              ...
              <div className="text-center md:text-left text-white pt-4">
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-4">
                  ...
                  <span className="px-3 py-1 bg-white/20 backdrop-blur-md rounded-lg text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 border border-white/10">
                    <Calendar className="w-3.5 h-3.5" /> Joined {user?.createdAt ? new Date(user.createdAt).toLocaleDateString() : 'Recently'}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-4xl mx-auto px-6 mt-8 pb-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
            ...
            >
              ...
              <span className="text-2xl font-black text-slate-800">{user?.currentStreak || 0}</span>
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Day Streak</span>
            </motion.div>
            ...
            >
              ...
              <span className="text-2xl font-black text-slate-800">{user?.xpPoints || 0}</span>
            ...
            >
              ...
              <span className="text-2xl font-black text-slate-800">{user?.league || 'Bronze'}</span>
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">League</span>
            </motion.div>
            ...
            >
              ...
              <span className="text-2xl font-black text-slate-800">{user?.longestStreak || 0}</span>
              ...
              {activeTab === 'general' && (
                ...
                >
                  <ConfigSection
                    ...
                    onSave={() => handleSavePartial({ name, headline, bio, dob }, 'basic')}
                    isLoading={isLoading === 'basic'}
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 block">Full Name</label>
                        <div className="relative group">
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 block">Headline</label>
                        <div className="relative group">
                      ...
                      <div className="space-y-2 md:col-span-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 block">Bio / About Me</label>
                        <textarea
                          value={bio}
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 block">Date of Birth</label>
                        <div className="relative group">
              ...
              {activeTab === 'billing' && (
                ...
                >
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 block">Email Address</label>
                        <div className="relative group">
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 block">Wallet Balance</label>
                        <div className="relative group">
                          ...
                          <div className="w-full bg-slate-100 border border-slate-200 rounded-2xl pl-11 pr-4 py-3 text-slate-700 font-bold flex items-center justify-between">
                            <span>{user?.walletCurrency || '₹'} {user?.walletBalance || 0}</span>
                            <button className="text-purple-600 text-xs hover:underline flex items-center gap-1 font-bold uppercase">
              ...
              {activeTab === 'security' && (
                ...
                >
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 block">Current Password</label>
                        <div className="relative group">
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1 block">New Password</label>
                        <div className="relative group">
