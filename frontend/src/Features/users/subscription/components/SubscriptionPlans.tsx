...
import { Check, Sparkles, Shield, Zap, Star, Layout, Crown } from 'lucide-react';
...
const SubscriptionPlans = () => {
    ...
    useEffect(() => {
        const fetchPlans = async () => {
            ...
            try {
                ...
                setPlans(activePlans.sort((a: plan, b: plan) => a.price - b.price));
            } catch (error) {
    ...
    const getPlanIcon = (name: string) => {
        ...
        if (n.includes('basic') || n.includes('free')) return <Zap className="text-blue-500" size={24} />;
        if (n.includes('pro') || n.includes('plus')) return <Star className="text-purple-500" size={24} />;
        if (n.includes('premium') || n.includes('elite')) return <Crown className="text-amber-500" size={24} />;
                ...
                <main className="flex-1 overflow-y-auto custom-scrollbar p-6 lg:p-10">
                    <div className="max-w-6xl mx-auto space-y-12">
                        ...
                        <div className="text-center space-y-4 max-w-3xl mx-auto">
                            ...
                            >
                                Choose the perfect plan for <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600">your productivity</span>
                            </motion.h1>
                            ...
                            >
                                Welcome back, <span className="text-gray-900 font-bold">{(currentUser as any)?.name || 'Achiever'}</span>! Unlock exclusive features, personalized mentorship, and advanced tracking tools to accelerate your growth.
                            </motion.p>
                        </div>
                        ...
                        {loading ? (
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                {[1, 2, 3].map((i) => (
                                    <div key={i} className="bg-white rounded-3xl p-8 h-[500px] border border-gray-100 animate-pulse space-y-6">
                                        <div className="h-8 w-32 bg-gray-100 rounded-lg"></div>
                                        <div className="h-12 w-24 bg-gray-100 rounded-lg"></div>
                                        <div className="space-y-3">
                                            {[1, 2, 3, 4, 5].map((j) => (
                                                <div key={j} className="h-4 w-full bg-gray-50 rounded"></div>
                                            ))}
                                        </div>
                                    ...
                                    {plans.map((plan, index) => (
                                        <motion.div
                                            ...
                                            className={`relative flex flex-col bg-white rounded-[2rem] p-8 transition-all duration-300 border border-gray-100 shadow-sm hover:shadow-2xl group ${
                                                plan.name.toLowerCase().includes('pro') ? 'ring-2 ring-indigo-500 ring-offset-4 ring-offset-transparent' : ''
                                            }`}
                                        >
                                            ...
                                            <div className="flex justify-between items-start mb-6">
                                                <div className={`p-3 rounded-2xl group-hover:scale-110 transition-transform duration-300 ${getPlanAccent(plan.name)}`}>
                                                    {getPlanIcon(plan.name)}
                                                </div>
                                                <div className="text-right">
                                                    <span className="block text-sm font-bold text-gray-400 tracking-wider uppercase">{plan.billingCycleDays} Days</span>
                                                </div>
                                            </div>

                                            <div className="space-y-4 mb-8">
                                                <h2 className="text-2xl font-extrabold text-gray-900 group-hover:text-indigo-600 transition-colors">{plan.name}</h2>
                                                <div className="flex items-baseline gap-1">
                                                    <span className="text-4xl font-black text-gray-900">₹{plan.price}</span>
                                                    <span className="text-gray-400 font-medium">/{plan.billingCycleDays}d</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex-1 space-y-6 mb-10">
                                                <div className="space-y-3">
                                                    <p className="text-sm font-bold text-gray-400 uppercase tracking-widest">Included Features</p>
                                                ...
                                                {plan.limitations && Object.keys(plan.limitations).length > 0 && (
                                                    <div className="space-y-3 pt-4 border-t border-gray-50">
                                                        <p className="text-sm font-bold text-gray-400 uppercase tracking-widest">Capacity & Limits</p>
                                                        <div className="grid grid-cols-2 gap-2">
                                                            {Object.entries(plan.limitations).map(([key, value], i) => (
                                                                <div key={i} className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                                                                    <p className="text-[10px] font-bold text-gray-400 uppercase truncate">{key.replace(/([A-Z])/g, ' $1')}</p>
                                                                    <p className="text-lg font-black text-gray-900">{value} per  month</p>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </div>
                        ...
                        <div className="text-center py-10">
                            <p className="text-gray-500">
                                Have questions about our plans? <a href="#" className="text-blue-600 font-bold hover:underline">Contact our support team</a>
                            </p>
