...
const VideoCallPage: React.FC = () => {
    ...
    useEffect(() => {
        if (remoteStream && userVideo.current) {
            console.log(
                'LOG: [Common] Syncing remoteStream to userVideo element.'
            );
    ...
    useEffect(() => {
        if (stream && myVideo.current) {
            console.log(
                'LOG: [Common] Syncing local stream to myVideo element.'
            );
    ...
    useEffect(() => {
        ...
        if (isMentor) {
            onUserConnected(({ userId, socketId }) => {
                console.log(
                    "LOG: [Mentor] 'user-connected' received from:",
                    userId,
                    socketId
                );
                ...
                if (
                    ...
                    console.log(
                        'LOG: [Mentor] Already connected to a peer. Skipping redundant call.'
                    );
                    return;
                }
        ...
        onSignal(({ signal, from }) => {
            console.log(
                "LOG: [Common] 'signal' received. Type:",
                signal.type || 'candidate/other'
            );

            if (signal.type === 'offer') {
                if (connectionRef.current) {
                    console.log(
                        'LOG: [User] Received NEW Offer. Destroying old peer to restart.'
                    );
    ...
    const callUser = (userSocketId: string) => {
        console.log(
            'LOG: [Mentor] callUser() called. checking streamRef...',
            streamRef.current
        );
        if (!streamRef.current) {
            console.error(
                'LOG: [Mentor] ABORTING callUser because stream is null!'
            );
            return;
        }
        ...
        if (connectionRef.current) {
            console.log(
                'LOG: [Mentor] Found existing peer. Destroying before new call.'
            );
            connectionRef.current.destroy();
            connectionRef.current = null;
        }
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                            <span
                                className={`w-2 h-2 rounded-full ${callAccepted ? 'bg-green-500' : 'bg-yellow-500 animate-pulse'}`}
                            />
